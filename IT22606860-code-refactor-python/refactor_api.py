...
def refactor():
    """Main refactoring endpoint - Now uses AST-based refactoring with LLM suggestions"""
    start_time = time.time()
    ...
    try:
        ...
        if use_ast:
            ...
            if not result['success']:
                return jsonify({
                    'success': False,
                    'message': f'AST refactoring failed: {result.get("error")}',
                    'original_code': code
                }), 500
            ...
            response = {
                ...
                'message': 'Code refactored successfully using AST'
            }
            
            print(f"[COMPLETE] {processing_time:.0f}ms - AST Method")
